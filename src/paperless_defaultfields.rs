...
use crate::paperless::{get_default_fields, update_document_default_fields};
use crate::types::{DefaultField, Document, Mode, PaperlessDefaultFieldType};
...
async fn construct_document_type_prompt(
    client: &Client,
    base_url: &str,
) -> Result<String, Box<dyn StdError + Send + Sync>> {
    let document_types =
        get_default_fields(client, base_url, PaperlessDefaultFieldType::DocumentType).await;
...
async fn construct_correspondent_prompt(
    client: &Client,
    base_url: &str,
) -> Result<String, Box<dyn StdError + Send + Sync>> {
    let document_types =
        get_default_fields(client, base_url, PaperlessDefaultFieldType::Correspondent).await;
...
pub async fn extract_default_fields(
    ollama: &Ollama,
    model: &str,
    prompt_base: &String,
    client: &Client,
                    ...
                    match extract_json_object(&res.response) {
                        Ok(json_str) => {
                            ...
                            match serde_json::from_str(&json_str) {
                                Ok(json) => {
                                    update_document_default_fields(
                                        client,
                                        document.id,
                                        &fields,
                                        json,
...
pub fn determine_if_type_exists(client: &Client, base_url: &str) {
    //TODO:
}
